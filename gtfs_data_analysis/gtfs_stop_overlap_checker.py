...
def get_minute_status_location(minute, block_segments, layover_threshold, trips_info):
    ...
    for (tid, tstart, tend, rname, dirid, start_stp, end_stp) in trips_info:
        if tstart <= current_sec <= tend:
            active_trip = (tid, tstart, tend, rname, dirid, start_stp, end_stp)
            break

    if active_trip is not None:
        tid, tstart, tend, rname, dirid, start_stp, end_stp = active_trip
        tsub = block_segments[block_segments['trip_id'] == tid].sort_values('arrival_seconds')
...
stop_times = stop_times.merge(trips[['trip_id','block_id','route_id','route_short_name','direction_id']], on='trip_id', how='left')

stop_times['arrival_seconds'] = stop_times['arrival_time'].apply(time_to_seconds) % 86400
stop_times['departure_seconds'] = stop_times['departure_time'].apply(time_to_seconds) % 86400
...
blocks_serving_interest = stop_times[stop_times['stop_id'].isin(stops_of_interest)]['block_id'].unique()
filtered_stop_times = stop_times[stop_times['block_id'].isin(blocks_serving_interest)]
...
for b_id in all_blocks:
    b_data = filtered_stop_times[filtered_stop_times['block_id'] == b_id].copy()
    if b_data.empty:
        continue

    seg_columns = ['trip_id','route_short_name','direction_id','stop_id','arrival_seconds','departure_seconds','next_stop_id','next_arrival_seconds']
    ...
    block_df['time_str'] = block_df['minute'].apply(lambda x: f"{x//60:02d}:{x%60:02d}")
    ...
    block_df['stop_name'] = block_df['stop_id'].apply(lambda x: stop_name_map[x] if x in stop_name_map else "")

    block_df['block_id'] = b_id
    inactive_mask = block_df['status'] == "inactive"
    block_df.loc[inactive_mask, ['block_id','route_short_name','direction','stop_id','stop_name']] = ""
...
def create_per_stop_excels(stops_of_interest, block_dataframes, output_folder):
    ...
    for s_id in stops_of_interest:
        ...
        summary_df['time_str'] = summary_df['minute'].apply(lambda x: f"{x//60:02d}:{x%60:02d}")
        ...
        for b_id in blocks_serving_stop:
            ...
            presence_mask = (bdf['stop_id'] == s_id) & (bdf['status'] != 'inactive')
        ...
        summary_df['blocks_present_str'] = summary_df['blocks_present'].apply(lambda x: ", ".join(sorted(set(x))))
        summary_df['routes_present_str'] = summary_df['routes_present'].apply(lambda x: ", ".join(sorted(set(x))))
        summary_df['num_blocks'] = summary_df['blocks_present'].apply(lambda x: len(set(x)))
        summary_df['conflict'] = summary_df['num_blocks'].apply(lambda x: "Yes" if x > 1 else "No")

        stop_name = stop_name_map.get(s_id, "")
        safe_stop_name = "".join([c if c.isalnum() else "_" for c in stop_name]) or "UnknownStop"
        output_file = os.path.join(output_folder, f"stop_{s_id}_{safe_stop_name}_blocks.xlsx")

        with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
            summary_cols = ['minute','time_str','num_blocks','blocks_present_str','routes_present_str','conflict']
            summary_df[summary_cols].to_excel(writer, sheet_name='Summary', index=False)

            for b_id in blocks_serving_stop:
                bdf = block_dataframes[b_id]
                bdf[['minute','time_str','block_id','route_short_name','direction','stop_id','stop_name','status']].to_excel(
...
def create_summary_of_summaries(stops_of_interest, block_dataframes, output_folder):
    ...
    cluster_summary_df['time_str'] = cluster_summary_df['minute'].apply(lambda x: f"{x//60:02d}:{x%60:02d}")
    ...
    for s_id in stops_of_interest:
        ...
        cluster_summary_df[f"{safe_stop_name}_{s_id}_blocks_present_str"] = ""
        cluster_summary_df[f"{safe_stop_name}_{s_id}_routes_present_str"] = ""

    # Aggregate presence data across all stops
    for s_id in stops_of_interest:
        # Identify blocks serving this stop
        blocks_serving_stop = [b_id for b_id, bdf in block_dataframes.items() if (bdf['stop_id'] == s_id).any()]
        ...
        for b_id in blocks_serving_stop:
            ...
            for idx, row in present_minutes.iterrows():
                ...
                if row['route_short_name']:
                    cluster_summary_df.at[m, 'routes_present'].append(row['route_short_name'])
                    per_stop_routes[s_id][m].append(row['route_short_name'])

    # Convert the top-level lists into strings
    cluster_summary_df['blocks_present_str'] = cluster_summary_df['blocks_present'].apply(lambda x: ", ".join(sorted(set(x))))
    cluster_summary_df['routes_present_str'] = cluster_summary_df['routes_present'].apply(lambda x: ", ".join(sorted(set(x))))
    ...
    cluster_summary_df['num_blocks'] = cluster_summary_df['blocks_present'].apply(lambda x: len(set(x)))

    # Write out the cluster-level summary
    output_file = os.path.join(output_folder, "cluster_summary_of_summaries.xlsx")
    with pd.ExcelWriter(output_file, engine='openpyxl') as writer:
        # Define columns explicitly without 'conflict'
        base_cols = ['minute','time_str','num_blocks','blocks_present_str','routes_present_str']
        per_stop_cols = []
        for s_id in stops_of_interest:
            stop_name = stop_name_map.get(s_id, "UnknownStop")
            safe_stop_name = "".join([c if c.isalnum() else "_" for c in stop_name]) or "UnknownStop"
            per_stop_cols.extend([f"{safe_stop_name}_{s_id}_blocks_present_str", f"{safe_stop_name}_{s_id}_routes_present_str"])

        final_cols = base_cols + per_stop_cols
        cluster_summary_df[final_cols].to_excel(writer, sheet_name='Cluster_Summary', index=False)
