    ...
    for schedule_type, days in SCHEDULE_TYPES.items():
        ...
        for route_short_name in ROUTE_SHORT_NAMES:
            ...
            for direction_id in relevant_trips['direction_id'].unique():
                ...
                timepoints = STOP_TIMES[STOP_TIMES['trip_id'].isin(trips_in_direction['trip_id'])].copy()
                    ...
                    for time_window_name, (start_time, end_time) in TIME_WINDOWS[schedule_type].items():
                        ...
                        if not timepoints_filtered.empty:
                            ...
                            timepoints_filtered['On'] = timepoints_filtered.apply(
                                lambda row: 'X' if row['stop_sequence'] == row['last_stop_sequence'] else MISSING_TIME,
                                axis=1
                            )

                            # 'Off' column
                            timepoints_filtered['Off'] = timepoints_filtered.apply(
                                lambda row: 'X' if row['stop_sequence'] == row['first_stop_sequence'] else MISSING_TIME,
                                axis=1
                            )
                            ...
                            columns_order = [
                                ...
                                'Trip ID + Start Time', 'Scheduled Time', 'Actual Time',
                                'Stop Sequence', 'Stop ID', 'Stop Name', 'On', 'Off'
                            ]
